...
- `[F6: 명령입력] - arcweld - weaving`을 입력, 조건번호를 입력하고 `[ENTER]`키를 누릅니다.
...
- 다시 `[F6: 명령입력] - arcweld - arcon` 을 입력하고 off로 설정합니다.
...
(2)	수동모드에서 `[F2: 시스템] - 5: 초기화 - 3: 용도설정` 을 누르면 [그림 1.2.1]과 같이 로봇의 용도, 사용하고자 하는 용접기, 사용자키 및 입출력신호 할당을 설정할 수 있는 대화상자가 나타납니다.

(3)	[그림 1.2.1]은 아크 용접 설정 유효로, `[F2: 용접기 정보]` 에서 확인한 용접기 번호를 1번('**효성**')으로 선택한 경우입니다. 이 화면에서 `[F1: 용접기 설정]` 을 누르면 사용하고자 하는 용접기의 조건을 설정하는 대화상자로 진입합니다.

(4)	용접기 특성 파일의 자세한 설정은 [[4장 Arc용접기 설정](../../4_Setting/README.md)]을 참고하시기 바랍니다.
...
#### [일반]
인칭 속도(%): 
**(저속)**[1 ~ 50] %, **(고속)**[10 ~ 100] % : `[SHIFT]+[2]` (인칭), `[SHIFT]+[3]` (역인칭)키를 사용한 와이어의 인칭 및 역인칭 실행 시 와이어 송급 속도입니다.<br> 저속 및 고속(3초 이상 키를 누르고 있을 때) 동작 시 송급 속도를 설정합니다.

`[GUN]`키 상태 출력신호: 
티치펜던트 `[GUN]`키의 현재 상태를 출력하기 위한 신호를 설정합니다.

`[GUN]`키 제어 불능 입력신호: 
외부에서 `[GUN]`키의 on/off를 설정할 수 있는 입력신호를 할당합니다. 해당 신호가 할당되면 티치펜던트의 `[GUN]`키를 눌러 아크용접 on/off 상태를 변경할 수 없습니다. 이 기능을 이용하면 사용자 실수로 `[GUN]`키를 눌러 용접구간에서 용접을 하지 않는 문제를 방지할 수 있습니다.<br>(할당된 신호가 입력되면 `[GUN]`키의 LED가 off되며 로봇이 재생되는 경우 Arc 용접 구간에서 용접을 수행하지 않는 **Dry Run** 상태로 작업이 진행됨)
...
{% hint style="info" %}
경고의 경우 경고메시지 발생, 에러의 경우 에러메시지 발생 후 로봇이 정지합니다.
{% endhint %}

Arc 용접 I/V 변경 자동저장 설정: 
|`무효, 유효`|, **Arc용접 전류전압 변경** 대화상자에서 전류와 전압 값을 변경할 경우 그 값을 자동 저장할 것인지 여부를 결정합니다. 자세한 내용은 [[1.3.3 용접 중 전류/전압 변경기능]](../3_Convenient_functions/3_change_iv.md)을 참조하십시오.
...
수동모드 Arc 용접을 사용하기 위해서는 아래와 같이 설정되어 있어야 합니다.
 ...
 (1) `[F2: 시스템 - 4: 응용 파라미터 - 2: 아크용접`에서 **"수동모드 아크용접 설정"** 를 **'유효'** 로 설정
...
- `[F2: 시스템] - 4: 응용 파라미터 - 2: 아크용접`에서 **"고중량 토치용 진동 저감 기능 설정"** 항목 중 **'무효/용접점/전 구간'** 중 택일
...
|ARC OFF 검출시간|[0.3]초 <br>(범위: 0.0 ~ 10.0)|Arc용접 중 Arc 꺼짐에 대한 기준 시간을 설정합니다. 해당 시간 이상 Arc가 꺼질 경우 Arc off로 인식합니다.<br> 이 값이 작은 경우 Arc점화 실패가 자주 발생할 수 있습니다. 반면에 설정된 값이 너무 크면 Arc off 후에도 로봇 이동과 와이어 인칭이 계속 되는 시간이 증가하기 때문에 Arc off 후 로봇이 이동하는 거리와 와이어 돌출되는 길이가 증가합니다.|
...
| 지원 용접기 | 명칭 | 단위 | 범위 | 기본값 |
| :---: | :---: | :---: |:---: | :---: |
| 효성 | 용접 전류 | A | 40.0 ~ 350.0 | 100.0 |
| EWM | 송급 속도 | m/min | -0.0 ~ 25.0 | 3.1 |
| Fronius(TPS) | 용접 파워 | % | 0.0 ~ 100.0 | 100.0 |
</center>
    
</br>

### (5)	용접 전압 / 용접 전압 보정 / 용접 전압 옵셋 / 아크 길이 보정  
...
| 지원 용접기 | 명칭 | 단위 | 범위 | 기본값 |
| :---: | :---: | :---: |:---: | :---: |
| 효성(개별) | 용접 전압 | V | 10.0 ~ 38.0 | 10.0 |
| 효성(시너직) | 용접 전압 보정 | % | 50.0 ~ 150.0 | 50.0 |
| EWM | 용접 전압 옵셋 | VP | -10.0 ~ 10.0 | 2.0 |
| Fronius(TPS) | 아크 길이 보정 | % | -30.0 ~ 30.0 | 0 |
</center>
...
| 지원 용접기 | 명칭 | 단위 | 범위 | 기본값 |
| :---: | :---: | :---: |:---: | :---: |
| 모든 용접기 공통 | WCR 대기시간 | 초 | 1.0 ~ 10.0 | 2 |
...
### (10) 초기 용접 전류 / 용접 파워 / 송급 속도  
Arc 용접 시작 시 초기조건 유지시간 동안 출력할 용접전류를 설정합니다. 본 조건 용접전류 대비 %로 설정합니다.
<center>

| 지원 용접기 | 명칭 | 단위 | 범위 | 기본값 |
| :---: | :---: | :---: |:---: | :---: |
| 효성 | 초기 용접 전류 | A | 40.0 ~ 350.0 | 120 |
| Fronius | 초기 용접 파워 | % | 20 ~ 200 | 120 |
| EWM | 초기 송급 속도 | m/min | 0.0 ~ 25.0 | 3.72 |

</center>
    
</br>

### (11) 초기 용접 전압 / 용접 전압 보정/ 아크 길이 보정  
Arc 용접 시작 시 초기조건 유지시간 동안 출력할 용접전압를 설정합니다. 시너직 전압 대비 보정값으로 설정합니다.
<center>

| 지원 용접기 | 명칭 | 단위 | 범위 | 기본값 |
| :---: | :---: | :---: |:---: | :---: |
| 효성(개별) | 초기 용접 전압 | V | 10.0 ~ 38.0 | 10.0 |
| 효성(시너직) | 초기 용접 전압 보정 | % | 50.0 ~ 150.0 | 50.0 |
| EWM | 초기 용접 전압 옵셋 | VP | -10.0 ~ 10.0 | 2 |
| Fronius | 초기 아크 길이 보정 | %| -30.0 ~ 30.0 | 0 |
...
### (13) 제한치 초과 허용 시간  
용접 전류/전압의 제한치 초과 허용시간을 설정합니다. 이 시간 이상 용접 전류/전압이 제한치를 초과하는 경우 재시작을 수행합니다. 단, 재시작 횟수가 0인 경우에는 에러를 출력하고 로봇이 정지합니다. 재시작 방법 및 재시작 횟수 등 재시작 관련 기능은 용접보조조건에서 설정 가능합니다. ([5.6 용접 보조 조건 - 재기동] 참고) 이 시간이 0초로 설정되는 경우 Arc제한 감시기능을 사용하지 않습니다.
<center>
...
### 편의 기능
---
설정화면 하단의 F버튼으로 용접조건 설정 시 편의 기능을 제공합니다.
- `[F1: 선택]`  
여러 개의 조건이 있을 때 편집할 조건 번호를 입력하여 선택할 수 있습니다.

- `[F2: 설정값 초기화]`  
용접 조건 파라미터들을 초기값으로 변경합니다.

- `[F3: 용접 시퀀스]`  
버튼을 누르고 있는 동안 용접 시퀀스 차트를 표시합니다. 
'시작 조건' 탭에서 누를 경우 용접 시작 시퀀스를, '종료 조건' 탭에서 누를 경우 용접 종료 시퀀스를 표시합니다.

- `[F4: 비율 변환]`  
초기 시퀀스, 종료 조건의 특정 항목 선택 시 버튼이 활성화 됩니다. 변환 비율(%)을 입력할 경우 본 조건 대비 변환값을 자동 계산하여 입력합니다.  

|기준값(본 조건)|초기 조건|종료 조건|
|-|-|-|
|용접 전류 / 용접 파워 / 와이어 송급 속도|초기 용접 전류 / 용접 파워 / 송급 속도|종료 용접 전류 / 용접 파워 / 송급 속도 |
|용접 전압 / 용접 전압 보정/ 아크 길이 보정|초기 용접 전압 / 용접 전압 보정 / 아크 길이 보정|종료 용접 전압 / 용접 전압 보정 / 아크 길이 보정|

- `[F5: 시너직 선택]`  
시너직 설정을 지원하는 용접기인 경우 버튼이 표시 됩니다. 버튼을 클릭하면 시너직 코드를 설정하는 화면으로 진입합니다.
...
-	무효: 용접기에 시너직 데이터에 따라 송급속도, 전압 옵셋이 반영되어 용접을 수행
-	유효: 용접기에 저장된 Job을 사용하여 용접 작업이 수행됨
...
### (5)	동적 보정: [ 0 ] (범위: -40.0 ~ 40.0)  
동적 보정 값을 설정합니다. 이 값이 작으면 강하고 안정적인 Arc가 발생하지만 스패터 양이 증가하고, 이 값이 크면 부드러운 Arc발생하고 스패터 양이 줄어듭니다.
...
[그림 5.5.1]의 좌측 항목은 용접 보조조건 중 재시도 조건을 나타냅니다. 재시도 조건의 각 항목별 내용은 다음과 같습니다.

### (1)	RETRACT 시간: [0] 초 (범위: 0.00 ~ 10.00)  
...
동작모드가 재진입으로 설정 시, 재시도 할 때에 토치를 이동시키는 거리입니다.  
...
Arc 용접 진행 중에는 Arc 꺼짐, 용접 전류 및 전압의 제한치 초과, 가스 압력 저하, 와이어 부족, 냉각수 에러 등으로 인해 용접이 중단될 수 있습니다. 이 때 용접이 정지된 지점에서 다시 용접을 시작하면 용접이 안된 부분이 발생할 수 있습니다. 이러한 경우 재기동 기능은 용접이 수행되지 않은 부분을 보완하기 위해 오버랩하여 용접을 수행합니다.

용접이 중단된 이후, 별도의 조치 없이도 자동으로 재기동하거나 중단 원인을 제거한 후 다시 기동하면 일정 거리만큼 용접선을 따라 역방향으로 이동한 후 용접을 시작합니다. 즉 용접 중단 위치 근처에서 용접 수행이 중복되는 오버랩 구간이 생기게 됩니다. 이를 통해 용접이 안된 부분이 생기는 것을 막을 수 있습니다.
...
### (3)	중첩량: [ 5 ] mm (범위: 0.0 ~ 99.9)  
재기동 할 때 용접을 중첩하는 길이(오버랩 거리)입니다. 중첩 길이만큼 되돌아 간 후 다시 용접을 시작합니다. 

### (4)	이동속도: [ 50 ] mm/초 (범위: 1.0~150.0)  
오버랩을 시작하는 위치로 토치를 이동 시키는 속도를 설정합니다. [그림 5.5.4]의 ③~④ 구간의 이동 속도를 의미합니다.

### (5)	용접속도: [ 50 ] cm/분 (범위: 10.0~999.0)  
오버랩 시작 위치에서 종료 위치까지 중첩 용접을 할 때 로봇의 이동 속도를 설정합니다. [그림 5.5.4]의 ④에서 오버랩 거리만큼 용접하며 이동할 때의 속도입니다.
...
### (6)	오버랩 조건설정  
[그림 5.5.3]의 하단 항목은 Arc 용접 중 ArcOff(Arc꺼짐), Limit 초과, 가스Off(가스 압력 저하), 와이어Off(와이어 부족), 냉각수Off(냉각수 에러) 등의 원인으로 용접이 중단되었을 때 오버랩을 수행할 방법에 대해 설정합니다.  
...
```refp 2```는 벽방향을 결정하는 평면(벽방향을 포함하는 평면)을 기준으로 하여 해당 평면의 양쪽 공간 중 어느 쪽에 위빙궤적을 생성할 것인지를 설정하는 명령어입니다. 
    사용방법: 위빙을 수행할 면 쪽의 공간 상 임의의 점을 refp 2로 기록합니다. [그림 6.2.2]은 두 모재 사이의 공간 상에 참조점 2를 기록했을 때 설정되는 위빙 좌표계의 예시입니다. refp 2 명령만 사용하는 경우 로봇 좌표계의 Z축을 벽방향으로 하여 타방향이 설정됩니다.  
...
- ② `[F6: 명령입력] - arcweld - 다음 - refp`를 눌러 참조점 명령을 기록합니다.
...
`[F2: 시스템] - 4: 응용파라미터 - 2: 아크용접` 에서 **"아크 궤적 모니터링"** 을 **'유효'** 로 설정
...
`[F6: 명령입력] - arcweld - arccond` 로 JOB파일에 명령어를 입력합니다.
...
아크 용접 시 위빙을 하면 토치와 모재 사이의 거리에 변화가 발생합니다. 거리 변화에 의해 와이어 저항이 변하여 흐르는 전류가 변합니다. 즉 위빙 구간의 전류 변화로부터 위빙 면에서의 좌우 방향으로 보정할 거리를 계산하여 용접선을 추종할 수 있습니다.
...
```${cont_model}에서는 용접선 추정 & 전류차를 사용을 권장합니다.```   <br>
...
`[(우측 패널)창조정] - 선택 - 아크 센싱`로 진입하면 아크 센싱 모니터링 창이 실행됩니다. 이 항목은 아크 센싱 라이선스가 유효한 상태에서만 활성화 됩니다.
...
LVS기능을 사용하기 위해서 TCP와 LVS 센서좌표계 간 캘리브레이션이 선행되어야 합니다. 

${cont_model} 제어기는 자동캘리브레이션을 지원합니다.

지금부터 tcp-lvs센서 자동캘리브레이션 수행 방법을 살펴보겠습니다.

### (1) 캘리브레이션 시편 준비

3mm 두께의 15cm 길이의 Lap조인트 시편을 준비하십시오. 
...
{% hint style="info" %}
Scansonic, Full-V 센서의 경우 opt=20을 사용하면 정밀 캘리브레이션을 수행할 수 있습니다.
{% endhint %}

아래 그림과 같이 시편의 기준점에 TCP를 조그로 이동시키십시오. 캘리브레이션 시에는 Tool 좌표계 조그 사용을 권장합니다.

그림의 1, 2와 같이 와이어의 절반이 각 방향에서 시편 모서리에 오도록 위치시켜야 합니다.

토치 자세는 시편과 수직으로 위치해야 합니다. (Roll, Pitch 방향 모두 수직)

레이저 라인은 시편의 엣지 부분과 수직이 되도록 조그로 위치 하십시오.

이 상태 (토치가 시편과 수직으로 위치하고 레이저 라인은 시편의 엣지에 수직인 상태)에서 **[기록]** 을 눌러 `move` 명령어를 삽입합니다.

{% hint style="warning" %}
* 수평계를 이용하여 모든방향에서 캘리브레이션 시편에 수직으로 맞추십시오.
* 토치의 수직 정도와 레이저라인이 시편의 엣지에 수직하는 정도는 캘리브레이션 정확도에 영향을 미칩니다.
{% endhint %}
...
*그림 8.7.5. 바우머 센서 신호 할당*
</br>  

**센서 입력신호**에 지정한 시작 번호가 **최하위 비트** 이고, 신호 번호가 커지는 방향이 상위 비트입니다.
**센서 비트수**를 입력하면 끝 신호 번호가 자동으로 표시됩니다.  

여기서 **센서1:거리1**, **센서2:거리2**는 센서 원시값과 실제 거리의 대응값 2개를 지정하는 설정입니다. 왼쪽 항목이 통신으로 수신되는 실수값, 오른쪽 항목이 그에 대응하는 실제 거리(mm)입니다.
이 두 점을 지나는 직선으로 선형 보간하여 현재 수신값을 거리로 환산합니다.
```거리 = 거리1 + (거리2 - 거리1) / (센서2 - 센서1) * (현재 센서값 - 센서1)```  

특히 **센서 비트수**가 **32**일 때는 센서가 거리값을 실수 그대로 전송하는 경우, **센서1**와 **센서2**를 동일한 값(예: 둘 다 0)으로 설정합니다.
이때 선형 보간을 수행하지 않고 수신된 32비트를 실수로 해석하여 거리값으로 사용하므로, **거리 1**, **거리 2** 설정은 무시됩니다.  

cf) **센서 비트수**가 **16**인 상태에서 같은 값으로 설정하면 실수 해석이 아니라, 수신값과 무관하게 항상 **거리1** 값이 출력됩니다.  


#### 최소 통신 사양

본 기능을 정상적으로 수행하기 위해서 통신 속도는 최소 5ms가 되어야 합니다. 당사 내장 이더넷 통신을 사용할 경우 RPI를 5(ms)로 설정하시기 바랍니다.
...
{% hint style="warning" %}
기능 사용전 툴-센서 간 캘리브레이션(8.7.2절 참조)이 수행되어 있지 않으면 비정상적인 포즈가 저장됩니다.
{% endhint %}
...
*그림 8.7.3.3 다양한 형상에서의 스캔 모드(scan)*<br/>  
