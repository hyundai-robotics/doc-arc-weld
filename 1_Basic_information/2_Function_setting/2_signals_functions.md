...
| 항목 | 설명 |
|------|------|
|입력신호|<br> 외부에서 `[GUN]`키의 on/off를 설정할 수 있는 입력신호를 할당합니다. 해당 신호가 할당되면 티치펜던트의 `[GUN]`키를 눌러 아크용접 on/off 상태를 변경할 수 없습니다. 이 기능을 이용하면 사용자 실수로 `[GUN]`키를 눌러 용접구간에서 용접을 하지 않는 문제를 방지할 수 있습니다.<br>(할당된 신호가 입력되면 `[GUN]`키의 LED가 off되며 로봇이 재생되는 경우 Arc 용접 구간에서 용접을 수행하지 않는 **Dry Run** 상태로 작업이 진행됨)|
