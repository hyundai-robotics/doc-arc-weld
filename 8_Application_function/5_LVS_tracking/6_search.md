...
LVS는 search 기능을 제공하며 tracking 전 search가 선행되어야 합니다.

- `search` : 시작점을 탐색하고 시작점으로 TCP가 이동하면서 추종할 위치를 설정 간격마다 버퍼에 저장하고 트래킹을 준비합니다.
- `step_search` : 다단비드 검출, 단차 검출 

search를 수행하면 탐색을 수행하며 옵션에 따라 다음과 같이 동작합니다.

(1) 레이저 위

레이저의 위치로 TCP가 이동하면서 버퍼에 추종점들을 저장하고 트래킹 준비를 완료합니다.

(2) 검출

탐색 방향으로 탐색거리만큼 이동하고 센싱이 불가한 시점을 검출하고 그 직전 위치로 TCP가 이동하면서 버퍼에 추종점들을 저장하고 트래킹 준비를 완료합니다.
...
```python
  ...
  lvs search, cnd=1, seam=1, sp=po_100 #트래킹 준비 완료
```
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align:left">
        ...
        '레이저 위' : lvs의 레이저 위치로 이동하면서 버퍼에 목표위치들을 저장합니다.<br>
        '검출' : 탐색방향으로 센싱 불가 점을 검출한 후 검출 직전 위치로 이동하면서 버퍼에 목표위치들을 저장합니다.
      </td>
    </tr>
    <tr>
...
find_flag에 변수를 할당한 경우 search 성공 시 1, 실패 시 0이 저장됩니다. 서치 실패시 lvs 명령어는 실행완료처리 되므로 주의하십시오.
