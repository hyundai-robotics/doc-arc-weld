...
LVS 용접선 찾기 및 추적 기능은 ```lvs``` 명령어를 통해 수행하며 TP의 `[F6: 명령입력] - arcweld - lvs` 를 입력하여 명령어를 입력할 수 있습니다.

명령어의 구성은 다음과 같습니다.

```python
  lvs 기능인자 cnd=조건번호, seam=센싱하고자 하는 프로파일 번호, sp=센싱된 위치의 포즈변수, mp=마스터 기준 포즈변수, ms=마스터 대비 현재 센싱위치 쉬프트변수, find_flag=플래그 변수
```
  ...
  <tr>
    ...
    <tr>
      ...
      <td>
        ...
        track 명령어 실행 중에는 최종 용접위치에 레이저가 오도록하는 위치가 포즈변수로 저장됩니다.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="2">opt</td>
      <td>
        자동 캘리브레이션(auto_calib) 명령어 사용 시 이 값을 0으로 두어야 합니다.<br>
        Scansonic, Full-V LVS의 경우 20을 지정하면 정밀 자동 캘리브레이션을 수행합니다.
      </td>
    </tr>
    <tr>
      <td colspan="2">플래그 변수</td>
      <td>
        이 파라미터에 변수를 지정하면 다음을 의미합니다. 
        1. Search <br>
