...
툴을 정렬시켰다면 와이어 끝을 캘리브레이션 평면 끝에 위치시킵니다. 툴 기준 X-Y 방향으로 교시할 때 레이저 포인터가 모서리 끝에 타고 움직일 수 있도록 RZ 값을 조정합니다.
...
TCP 끝을 캘리브레이션 평면의 한 쪽 꼭짓점에 위치하도록 합니다. 또 레이저 포인터는 캘리브레이션 평면 안쪽에 위치해야 합니다.
...
하단 패널에서 **[명령 입력 → arcweld → lps]** 를 클릭하여 하기 명령어를 삽입합니다.
...
**[시스템 → 4: 응용 파라미터 → 6: 레이저 포인터 센싱 → 2: 캘리브레이션]** 에 진입하여 캘리브레이션 결과를 확인할 수 있습니다. "캘리브레이션 완료" 칸에 "2"로 바뀌면 보간까지 모두 완료되었음을 의미합니다.
