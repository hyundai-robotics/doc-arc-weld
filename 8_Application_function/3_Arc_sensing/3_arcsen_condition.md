...
(6) 상하 센싱 기준 전류: [0 ~ 3000]  
    상하방향 센싱을 수행할 때 기준이 되는 전류를 설정합니다. 
    이 설정에 의해 아크센싱 용접선 추종시 토치 높이가 결정됩니다. 
    ```0으로 설정하는 경우 초기 구간 전류의 평균값이 기준이 됩니다.```
    ```0으로 설정 시 용접 시작 부위에 가접이 있는 경우 의도한 바와 다른 높은 초기전류가 기준값이 되므로 주의하십시오.```

(7) 아크센싱 타입 선택: <용접선, 전류차, 전류차+Gap, 용접선 추정 & 전류차>  
    ```Hi6에서는 용접선 추정 & 전류차를 사용을 권장합니다.``` 
...
---
아크센싱 조건-사용자 대화상자에서 아래와 같은 아크센싱 조건 (advanced) 탭은 아래와 같습니다. 
