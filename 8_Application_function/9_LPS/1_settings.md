...
{% hint style="warning" %}
툴 좌표계를 설정 후에 최대한 표면과 수직하게 설치해야 오차가 가장 작습니다.
{% endhint %}  
...
0. 로봇의 tcp를 평면에 최대한 가깝게 두고 다음을 실행합니다.(좌측 그림)
1. `SET` 키를 길게 누르고, 🔼 키를 눌러 `out-1`을 선택합니다.
2. `ENT` 키를 누르고 ▶️ 키로 function `b`를 선택합니다.
3. `ENT` 키를 누르고 ▶️ 키로 function `b-0`를 선택합니다.
4. `ENT` 키를 누르고 ▶️/🔼 키로 포인트 1의 입력값1을 설정합니다.  
  (이때 `ZERO`키를 누르면 현재 측정값이 입력됩니다.)
5. `ENT` 키를 누르고 ▶️/🔼 키로 포인트 1의 표시값1을 설정합니다.  

6. 로봇을 **조그인칭**하여 센싱 범위 내에 평면과 최대한 멀리 두고 다음을 실행합니다.(우측 그림)
7. `ENT` 키를 누르고 ▶️/🔼 키로 포인트 2의 입력값2을 설정합니다.  
  (이때 `ZERO`키를 누르면 현재 측정값이 입력됩니다.)
8. `ENT` 키를 누르고 ▶️/🔼 키로 포인트 2의 표시값2을 설정합니다.
9. `ENT` 키를 눌러 설정을 등록하고, `SET` 키로 측정 상태로 돌아갑니다.  
<br>
